...
class ProjectViewSet(BaseViewSet):
    ...
    def partial_update(self, request, slug, pk=None):
        # Fallback: workspace admins can edit any project even if not a project member
        if not ProjectMember.objects.filter(
            ...
            pass  # workspace admin, proceed
        else:
            # verify project membership (decorator already checked admin/project-admin)
            if not ProjectMember.objects.filter(
                member=request.user,
                workspace__slug=slug,
                project_id=pk,
