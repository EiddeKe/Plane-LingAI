...
class UserProjectInvitationsViewset(BaseViewSet):
    ...
    def create(self, request, slug):
        ...
        # If the user was already part of workspace, mark as pending for re-request
        _ = ProjectMember.objects.filter(
            workspace__slug=slug, project_id__in=validated_project_ids, member=request.user
        ).update(is_active=True, is_pending=True)
        ...
        ProjectMember.objects.bulk_create(
            [
                ProjectMember(
                    ...
                    workspace=workspace,
                    created_by=request.user,
                    is_pending=True,  # requires admin approval
        ...
        return Response({"message": "Join request sent. Waiting for admin approval."}, status=status.HTTP_201_CREATED)
