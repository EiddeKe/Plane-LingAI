...
class ProjectMemberViewSet(BaseViewSet):
    ...
    def list(self, request, slug, project_id):
        # Get the list of active, approved project members for the project
        project_members = ProjectMember.objects.filter(
            project_id=project_id,
            workspace__slug=slug,
            member__is_bot=False,
            is_active=True,
            is_pending=False,  # exclude pending join requests
...
class UserProjectRolesEndpoint(BaseAPIView):
    ...
    def get(self, request, slug):
        project_members = ProjectMember.objects.filter(
            ...
            is_active=True,
            is_pending=False,  # exclude pending memberships
            member__member_workspace__workspace__slug=slug,
            member__member_workspace__is_active=True,
...
class ProjectPendingMembersEndpoint(BaseAPIView):
    @allow_permission([ROLE.ADMIN])
    def get(self, request, slug, project_id):
        pending_members = ProjectMember.objects.filter(
            project_id=project_id,
            workspace__slug=slug,
            is_pending=True,
            is_active=False,
        ...
        serializer = ProjectMemberRoleSerializer(pending_members, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)


class ProjectMemberApprovalEndpoint(BaseAPIView):
    @allow_permission([ROLE.ADMIN])
    def post(self, request, slug, project_id, pk):
        ...
        if action == "approve":
            project_member.is_pending = False
            project_member.is_active = True
            project_member.save()
            return Response({"message": "Member approved"}, status=status.HTTP_200_OK)
        else:
