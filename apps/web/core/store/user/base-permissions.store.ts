...
export interface IBaseUserPermissionStore {
  ...
  pendingProjectIds: string[]; // project IDs with pending join requests
  // computed helpers
  ...
  isProjectJoinPending: (projectId: string) => boolean;
  // actions
  ...
  joinProject: (workspaceSlug: string, projectId: string) => Promise<{ is_pending: boolean } | void>;
...
export class BaseUserPermissionStore implements IBaseUserPermissionStore {
  ...
  /**
   * @description Returns whether a project join request is pending approval
   * @param { string } projectId
   * @returns { boolean }
   */
  isProjectJoinPending = computedFn((projectId: string): boolean => {
    return this.pendingProjectIds.includes(projectId);
  });
  ...
  /**
   * @description Joins a project (requires admin approval)
   * @param { string } workspaceSlug
   * @param { string } projectId
   * @returns { Promise<{ is_pending: boolean } | void> }
   */
  joinProject = async (workspaceSlug: string, projectId: string): Promise<{ is_pending: boolean } | void> => {
    try {
      const response = await userService.joinProject(workspaceSlug, [projectId]);
      // Backend now creates pending memberships (is_pending=true, is_active=false)
      // Do NOT grant immediate permissions
      if (response?.message?.includes("Waiting for admin approval")) {
        runInAction(() => {
          if (!this.pendingProjectIds.includes(projectId)) {
            this.pendingProjectIds.push(projectId);
          }
        });
        return { is_pending: true };
      }
      // Fallback for legacy behavior (e.g., admin-invited users)
