...
export const ProjectAuthWrapper = observer(function ProjectAuthWrapper(props: IProjectAuthWrapper) {
  ...
  // store hooks
  const { fetchUserProjectInfo, allowPermissions, getProjectRoleByWorkspaceSlugAndProjectId, isProjectJoinPending } = useUserPermissions();
  ...
  const hasPendingJoinRequest = isProjectJoinPending(projectId);
  ...
  const handleJoinProject = async () => {
    ...
    try {
      const result = await joinProject(workspaceSlug, projectId);
  ...
  if (!isProjectLoading && hasPermissionToCurrentProject === false) {
    // Show pending approval message if user has a pending join request
    if (hasPendingJoin || hasPendingJoinRequest) {
      return (
