...
export const useCycleMenuItems = (props: UseCycleMenuItemsProps): MenuResult => {
  ...
  const { cycleDetails, isEditingAllowed, isDeletionAllowed, ...handlers } = props;

  const isArchived = !!cycleDetails?.archived_at;
  const isCompleted = cycleDetails?.status?.toLowerCase() === "completed";
  const canDelete = isDeletionAllowed ?? isEditingAllowed;
  ...
  const items = [
    ...
    factory.createDeleteMenuItem(handlers.handleDelete, canDelete && !isCompleted && !isArchived),
...
export const useModuleMenuItems = (props: UseModuleMenuItemsProps): MenuResult => {
  const factory = useQuickActionsFactory();
  const { moduleDetails, isEditingAllowed, isDeletionAllowed, ...handlers } = props;
  ...
  const items = [
    ...
    factory.createRestoreMenuItem(handlers.handleRestore, isEditingAllowed && isArchived),
    factory.createDeleteMenuItem(handlers.handleDelete, canDelete && !isArchived),
  ].filter((item) => item.shouldRender !== false);
